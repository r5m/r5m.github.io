		...
		return window.AppData.objCur = {
			...
			beforeActivate: function(){
				...
				arrayUtil.forEach(self.currencyPicker.getChildren(),function(_cur){				
					...
					_cur.set('checked', ( _cur.id == cur ) ? true : false)
				
				});
				
				registry.byId('currencyInput').set('placeHolder', cur)
			},
			init: function(){
				var self = this
...
define({
	root: {
		...
		welcome2: 'R5M Finance is a free personal finance manager for mobile devices',
...
define({
	    ...
	    menu	: 'Меню',
		welcome	: 'R5M Finance',
		welcome2: 'Добро пожаловать в R5M Finance - менеджер личных финансов для мобильных устройств.',
...
define({
	root: {
		title	: "R5M Finance",
	    r5mfIs	: 'R5M Finance is free cross-platform mobile finance manager by R5M.',
	    menu	: 'Menu',
	    updates	: 'Updates:',
	    ...
	    thanks	: 'Thanks to developers of:',
...
define({
		title	: "R5M Finance",
	    r5mfIs	: 'R5M Finance - бесплатный кросс-платформенный меннеджер личных финансов для мобильных устройств',
	    menu	: 'Меню',
	    updates	: 'Обновления:',
	    symba	: 'Note for Symbian (Nokia belle) users: ..Symbian, as you might know, also works on magic. So it\'s all about magic*magic = magic^2! ;-)',
	    thanks	: 'Авторы выражают благодарность разработчикам:',
		...
		_getData:function(type){
			...
			arrayUtil.forEach(data, function(item){
				...
				if(chartKeys[tags] == undefined){
					chartData.push ({ x:1, y:Math.abs( parseFloat(item.amountHome) ), text: '<span class="chart-label"><span class="chart-label-text">1 * '+tagsLabels + "</span>" + '&nbsp;<span class="chart-label-amount">'+ getMoney( Math.abs(item.amountHome).toFixed(2) ) +" ("+(Math.abs(item.amountHome / (sum != 0 ? sum : 0.001))*100).toFixed(2)+"%)</span></span>"});
					chartKeys[tags] = {i : chartData.length-1, l: 1}
				}else {
					chartKeys[tags].l ++;
					chartData[chartKeys[tags].i].y += Math.abs(parseFloat(item.amountHome)), chartData[chartKeys[tags].i].text = '<span class="chart-label"><span class="chart-label-text">' + chartKeys[tags].l + ' * '+tagsLabels + '</span>&nbsp;<span class="chart-label-amount">' + getMoney( Math.abs( chartData[ chartKeys[tags].i ].y).toFixed(2))+ " ("+ (Math.abs( chartData[ chartKeys[tags].i ].y / (sum != 0 ? sum : 0.001) ) * 100 ).toFixed(2)+"%)</span></span>"
...
define({
	...
	clearRemote	: 'Удалить все данные в Dropbox',
        ...
        return declare(null, {
            ...
            removeItem: function(removedFrom){
                try{
					...
					console.log('remove item', removedFrom);
					this.store.remove( removedFrom )//this.getItem(removedFrom) )
            ...
            _onloadSavedData: function(data){
                ...
                */
				...
				window.AppData.tagsStore = new Memory({ idProperty:'id', data: tags});					
...
'url:money/views/theme.html':"<div class=\"mblBackground\">\n\t<div \n\t\tdata-dojo-attach-point=\"heading\"\n\t\tdata-dojo-type=\"dojox/mobile/Heading\" data-dojo-props=\"label:'${nls.title}'\">\n\t\t<button data-dojo-type=\"dojox/mobile/ToolBarButton\"\n            data-dojo-attach-point=\"backButton\"\n            data-dojo-props=\"arrow: 'left', target: 'settings', transitionDir: -1\"\n            style=\"float:left\"\n            class=\"backButton\">${nls.back}</button>\n\t</div>\n\t<div \n\t\tdata-dojo-type=\"money/WheelScrollableView\" data-dojo-props=\"height:'auto'\"\n\t\tdata-dojo-attach-point=\"viewContainer\">\n\t\t<div data-dojo-type=\"dojox/mobile/RoundRectCategory\" data-dojo-props=\"label:'${nls.theme}'\"></div>\n\t\t<ul data-dojo-type=\"dojox/mobile/RoundRectList\" data-dojo-props=\"select: 'single'\">\n\t\t\t<div \n\t\t\t\tdata-dojo-attach-point=\"ios7\" \n\t\t\t\tdata-dojo-type=\"dojox/mobile/ListItem\" \n\t\t\t\tdata-dojo-props=\"label: 'IOS7', checked: true\"></div>\n\t\t\t<div \n\t\t\t\tdata-dojo-attach-point=\"iphone\" \n\t\t\t\tdata-dojo-type=\"dojox/mobile/ListItem\" \n\t\t\t\tdata-dojo-props=\"label: 'IOS Classic'\"></div>\t\t\t\n\t\t\t\n\t\t</ul>\n\t\n\t</div>\t\t\t\n</div>\n",
